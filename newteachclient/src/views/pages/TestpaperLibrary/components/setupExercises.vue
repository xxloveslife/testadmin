  ...
  methods: {
    ...
    setupExercises() {
      if (this.openPopubItem === '成功') {
        var data = {}
        data.paper_id = this.tempItem.id
        data.name = this.setupExercisesForm.name
        console.log(1, this.currentLib, 1)
        ...
        console.log(data)
        if (this.setupExercisesForm.name === '') {
          this.$message.error('练习名称不能为空')
        } else {
          if (this.classRoomList === null || this.fitGradeValue === -1) {
            ...
            if (this.checkboxGroup.length) {
              ...
              if (this.timeValue === '') {
                ...
                // 发请求
                this.$store
                  .dispatch('testlibrary/setUpExercises', data)
                  .then((res) => {
                    console.log(res)
                    if (res.code === 0) {
                      console.log(res)
