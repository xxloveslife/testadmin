import { getTestLibraryCategory, getSysTestLibraryCategory, getTestPaperLib, getSysTestPaperLib } from '../../api/testLibrary'
const state = {
    library_result: {},
    test_paper_lib: {}
}
...
const actions = {
    ...
    getTestPaperLibResult({ commit }, quesCondition) {
        return new Promise((reslove, reject) => {
            getTestPaperLib(quesCondition).then(res => {
                // 修改状态码    
                if (res.code === 0) {
    ...
    getSysTestPaperLibResult({ commit }, quesCondition) {
        return new Promise((reslove, reject) => {
            getSysTestPaperLib(quesCondition).then(res => {
                // 修改状态码    
                if (res.code === 0) {
                    commit('SET_TEST_PAPER_LIB', res.data)
                }
